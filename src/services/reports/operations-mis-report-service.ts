...
import type {
  OperationsMisReportQueryParams,
  OperationsMisReportResponse,
} from "@/types/reports/operations-mis-report";
...
function appendParams(
  ...
  params?: OperationsMisReportQueryParams,
) {
...
export const operationsMisReportService = {
  async getReport(
    params?: OperationsMisReportQueryParams,
  ): Promise<OperationsMisReportResponse> {
    const queryParams = new URLSearchParams();
    appendParams(queryParams, params);
    const response = await apiFetch(
      `${API_URL}/report/operations-mis?${queryParams.toString()}`,
      { headers: authHeaders() },
    );
    if (!response.ok) {
      throw new Error(await readError(response, "Failed to fetch Operations MIS report"));
    }
  ...
  async exportXlsx(
    ...
    appendParams(queryParams, params);
    const response = await apiFetch(
      `${API_URL}/report/operations-mis/export?${queryParams.toString()}`,
      { headers: authHeaders() },
    );
    if (!response.ok) {
      throw new Error(await readError(response, "Failed to export Operations MIS report"));
    }
    return {
      blob: await response.blob(),
      filename: parseFilename(response, "operations-mis-report.xlsx"),
    };
  },
};
