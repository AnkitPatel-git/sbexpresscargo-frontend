...
export const serviceablePincodeService = {
    ...
    async downloadImportTemplate(): Promise<{ blob: Blob; filename: string }> {
        const response = await apiFetch(
            `${API_BASE_URL}/utilities/serviceable-pincodes/import/template`,
            { headers: bearerHeaders(false) },
        );
        if (!response.ok) {
        ...
        const cd = response.headers.get('content-disposition');
        let filename = 'serviceable-pincodes-import-template.xlsx';
        const match = cd?.match(/filename="?([^";\n]+)"?/i);
        if (match?.[1]) filename = match[1].trim();
        const blob = await response.blob();
        return { blob, filename };
    ...
    async importServiceablePincodesFromExcel(file: File): Promise<{
        ...
        const response = await apiFetch(`${API_BASE_URL}/utilities/serviceable-pincodes/import`, {
            method: 'POST',
            headers: bearerHeaders(false),
            body: formData,
        });
