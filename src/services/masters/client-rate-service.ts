...
export const clientRateService = {
    async getClientRates(params?: {
        ...
        const response = await apiFetch(`${API_URL}/client-rate-master?${queryParams.toString()}`, {
            headers: {
    ...
    async getClientRateById(id: number): Promise<ClientRateSingleResponse> {
        const response = await apiFetch(`${API_URL}/client-rate-master/${id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
            },
        });
    ...
    async createClientRate(data: ClientRateFormData): Promise<ClientRateSingleResponse> {
        const response = await apiFetch(`${API_URL}/client-rate-master`, {
            method: 'POST',
            headers: {
