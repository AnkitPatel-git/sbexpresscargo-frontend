...
export const vendorService = {
    async getVendors(params?: {
        ...
        const response = await fetch(`${API_URL}/vendor-master?${queryParams.toString()}`, {
            headers: {
    ...
    async getVendorById(id: number): Promise<VendorSingleResponse> {
        const response = await fetch(`${API_URL}/vendor-master/${id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
            },
        });
    ...
    async createVendor(data: VendorFormData): Promise<VendorSingleResponse> {
        const response = await fetch(`${API_URL}/vendor-master`, {
            method: 'POST',
            headers: {
