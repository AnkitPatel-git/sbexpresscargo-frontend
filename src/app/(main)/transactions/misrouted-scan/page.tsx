    ...
    <div className="rounded-lg border border-border/80 bg-card p-4 shadow-[0_1px_3px_rgba(23,42,69,0.08)] lg:p-5">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-1 rounded-md border border-border p-1">
          <PermissionGuard permission="transaction.misrouted-scan.create">
            <Button
              ...
              title="Add"
              onClick={() => router.push("/transactions/misrouted-scan/create")}
            >
          ...
          <Button
            ...
            className="h-8 w-8 text-primary"
            title="Refresh"
            onClick={() => queryClient.refetchQueries({ queryKey: ["misrouted-scans"], type: "active" })}
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          />
          <PermissionGuard permission="transaction.misrouted-scan.create">
            <Button type="button" className="h-9 rounded-md px-3" onClick={() => router.push("/transactions/misrouted-scan/create")}>
              <Plus className="mr-1 h-4 w-4" /> Add Misrouted Scan
            </Button>
          </PermissionGuard>
        </div>
      </div>
        ...
        <Table className="min-w-[760px] border-0">
          ...
          <TableBody>
            {isLoading ? (
              ...
              filteredRows.map((scan: MisroutedScan, index) => (
                <TableRow key={scan.id} className={cn("border-border", index % 2 === 1 ? "bg-muted/40" : "bg-card")}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">{scan.eventType}</Badge>
                  </TableCell>
