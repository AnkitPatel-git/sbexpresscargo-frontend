...
import { partywiseSalesSummaryService } from "@/services/reports/partywise-sales-summary-service";

type PartywiseSalesFilters = {
  customerGroupId?: number;
};

const DEFAULT_FILTERS: PartywiseSalesFilters = {};
...
export default function PartywiseSalesSummaryPage() {
  ...
  const [appliedFilters, setAppliedFilters] = useState<PartywiseSalesFilters>(DEFAULT_FILTERS);
  const [draftFilters, setDraftFilters] = useState<PartywiseSalesFilters>(DEFAULT_FILTERS);
  ...
  const { data: groupData } = useQuery({
    queryKey: ["partywise-sales-customer-group-options"],
    queryFn: () =>
  ...
  const { data, isLoading, isFetching, refetch } = useQuery({
    ...
    queryFn: () =>
      partywiseSalesSummaryService.getReport({
        page,
        limit: 20,
  ...
  async function handleExport() {
    setExporting(true);
    try {
      const { blob, filename } = await partywiseSalesSummaryService.exportCsv({
        search: search || undefined,
        sortBy,
        sortOrder,
        customerGroupId: appliedFilters.customerGroupId,
      ...
      toast.success("Partywise Sales Summary exported");
    } catch (err) {
    ...
    >
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Partywise Sales Summary</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                void refetch();
                void queryClient.invalidateQueries({ queryKey: ["partywise-sales-summary"] });
              }}
            >
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={8} className="h-24 text-center text-muted-foreground">
                    Loading Partywise Sales Summary…
                  </TableCell>
                </TableRow>
              ) : rows.length === 0 ? (
                <TableRow>
