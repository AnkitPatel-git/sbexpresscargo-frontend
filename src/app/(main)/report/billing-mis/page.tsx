...
import { billingMisReportService } from "@/services/reports/billing-mis-report-service";
import type { BillingMisReportColumnKey } from "@/types/reports/billing-mis-report";
import { useAuth } from "@/context/auth-context";
import { MASTER_READ, hasMasterLookupForPortalTransaction } from "@/lib/portal-permissions";

type BillingMisFilters = {
  awbNo: string;
...
const DEFAULT_FILTERS: BillingMisFilters = {
  awbNo: "",
  forwardingAwb: "",
  ewaybillNumber: "",
  bookDateFrom: "",
...
const NUMERIC_COLUMNS = new Set<BillingMisReportColumnKey>([
  "invoiceValue",
  "qty",
  "actualWeight",
  "length",
  ...
  "gst",
  "invoiceAmount",
  "vendorChargeableWeight",
  "vendorCharges",
]);

const MULTILINE_COLUMNS = new Set<BillingMisReportColumnKey>([
  "length",
  "breadth",
  "height",
  "noOfBox",
...
export default function BillingMisReportPage() {
  const {
    isCustomerUser,
    defaultCustomerId,
  ...
  const { data: customerData } = useQuery({
    queryKey: ["billing-mis-report-customer-options"],
    queryFn: () =>
  ...
  const { data: serviceCenterData } = useQuery({
    queryKey: ["billing-mis-report-service-center-options"],
    queryFn: () =>
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["billing-mis-report", listParams],
    queryFn: () => billingMisReportService.getReport(listParams),
  });
  ...
  const displayColumns = (data?.columns ?? []) as BillingMisReportColumnKey[];
  ...
  const resetFilters = () => {
    const next = isCustomerUser && scopedCustomerId
  ...
  async function handleExport() {
    try {
      const { blob, filename } = await billingMisReportService.exportXlsx(listParams);
  ...
  return (
    <PermissionGuard
      permission="report.billing_mis.read"
      fallback={
        <div className="rounded-lg border border-border/80 bg-card p-6 text-sm text-muted-foreground">
          You do not have permission to view the Billing MIS report.
        </div>
      }
    >
      <div className="rounded-lg border border-border/80 bg-card p-4 shadow-[0_1px_3px_rgba(23,42,69,0.08)] lg:p-5">
        <div className="mb-4">
          <h1 className="text-lg font-semibold tracking-tight">Billing MIS Report</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Shipment billing breakdown with charge columns matching the standard billing MIS format.
          </p>
        </div>
          ...
          <div className="flex flex-wrap justify-start gap-2 lg:justify-end">
            <Dialog open={filtersOpen} onOpenChange={setFiltersOpen}>
              ...
              <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                  <DialogTitle>Filter Billing MIS Report</DialogTitle>
            ...
            <Button
              ...
              variant="outline"
              className="h-9 gap-2"
              onClick={() => queryClient.refetchQueries({ queryKey: ["billing-mis-report"], type: "active" })}
            >
