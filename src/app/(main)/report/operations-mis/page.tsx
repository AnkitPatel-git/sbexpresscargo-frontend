...
import { operationsMisReportService } from "@/services/reports/operations-mis-report-service";
import type { Vendor } from "@/types/masters/vendor";
import type { OperationsMisReportColumnKey } from "@/types/reports/operations-mis-report";
import { useAuth } from "@/context/auth-context";
import { MASTER_READ, hasMasterLookupForPortalTransaction } from "@/lib/portal-permissions";

type OperationsMisFilters = {
  awbNo: string;
...
const DEFAULT_FILTERS: OperationsMisFilters = {
  awbNo: "",
  forwardingAwb: "",
  ewaybillNumber: "",
  referenceNo: "",
...
const NUMERIC_COLUMNS = new Set<OperationsMisReportColumnKey>([
  "srNo",
  "odaDistance",
  "invoiceAmount",
  "noOfBoxes",
...
export default function OperationsMisReportPage() {
  ...
  const [appliedFilters, setAppliedFilters] = useState<OperationsMisFilters>(DEFAULT_FILTERS);
  const [draftFilters, setDraftFilters] = useState<OperationsMisFilters>(DEFAULT_FILTERS);

  const { data: pinnedVendorData } = useQuery({
    queryKey: ["operations-mis-report-vendor-pin", draftFilters.vendorId],
  ...
  const { data: customerData } = useQuery({
    queryKey: ["operations-mis-report-customer-options"],
    queryFn: () =>
  ...
  const { data: serviceCenterData } = useQuery({
    queryKey: ["operations-mis-report-service-center-options"],
    queryFn: () =>
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["operations-mis-report", listParams],
    queryFn: () => operationsMisReportService.getReport(listParams),
  });
  ...
  const displayColumns = (data?.columns ?? []) as OperationsMisReportColumnKey[];
  ...
  async function handleExport() {
    try {
      const { blob, filename } = await operationsMisReportService.exportXlsx(listParams);
      ...
      toast.success("Operations MIS report exported");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to export Operations MIS report");
    }
  }

  return (
    <PermissionGuard
      permission="report.operations_mis.read"
      fallback={
        <div className="rounded-lg border border-border/80 bg-card p-6 text-sm text-muted-foreground">
          You do not have permission to view the Operations MIS report.
        </div>
      }
    >
      <div className="rounded-lg border border-border/80 bg-card p-4 shadow-[0_1px_3px_rgba(23,42,69,0.08)] lg:p-5">
        <div className="mb-4">
          <h1 className="text-lg font-semibold tracking-tight">Operations MIS Report</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Pickup → dispatch → delivery movement and TAT tracking in the standard operations MIS format.
          </p>
        </div>
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_auto]">
          <div className="flex gap-2">
            <Input
              placeholder="Search AWB, customer, shipper, consignee, FWD..."
              value={searchInput}
          ...
          <div className="flex flex-wrap justify-start gap-2 lg:justify-end">
            <Dialog open={filtersOpen} onOpenChange={setFiltersOpen}>
              ...
              <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                  <DialogTitle>Filter Operations MIS Report</DialogTitle>
                  <DialogDescription>Filter shipments by booking and master data fields.</DialogDescription>
                </DialogHeader>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <DbAsyncSelect<Vendor>
                    queryKey={["operations-mis-report-vendor-options"]}
            ...
            <Button
              ...
              variant="outline"
              className="h-9 gap-2"
              onClick={() => queryClient.refetchQueries({ queryKey: ["operations-mis-report"], type: "active" })}
            >
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <Table className="min-w-[6000px] border-0">
            <TableHeader>
