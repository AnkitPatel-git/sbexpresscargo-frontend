...
export default function CountriesPage() {
    ...
    const [colFilters, setColFilters] = useState({ code: "", name: "", weightUnit: "", currency: "", isdCode: "" })
    ...
    const filteredRows =
        data?.data.filter((country: Country) => {
            if (colFilters.code && !country.code.toLowerCase().includes(colFilters.code.toLowerCase())) return false
            if (colFilters.name && !country.name.toLowerCase().includes(colFilters.name.toLowerCase())) return false
            if (colFilters.weightUnit && !(country.weightUnit || "").toLowerCase().includes(colFilters.weightUnit.toLowerCase())) return false
            if (colFilters.currency && !(country.currency || "").toLowerCase().includes(colFilters.currency.toLowerCase())) return false
            if (colFilters.isdCode && !(country.isdCode || "").toLowerCase().includes(colFilters.isdCode.toLowerCase())) return false
            return true
        }) ?? []
        ...
        <div className="rounded-lg border border-border/80 bg-card p-4 shadow-[0_1px_3px_rgba(23,42,69,0.08)] lg:p-5">
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Input placeholder="Search countries..." className="h-9 w-44 bg-background sm:w-52" value={search} onChange={(e) => setSearch(e.target.value)} />
                    <PermissionGuard permission="master.country.create">
                        <Button type="button" className="h-9 rounded-md px-3" onClick={handleCreate}><Plus className="mr-1 h-4 w-4" />Add Country</Button>
                    </PermissionGuard>
                </div>
            </div>
                ...
                <Table className="min-w-[920px] border-0">
                    <TableHeader>
                        ...
                        <TableRow className="border-b border-border bg-card hover:bg-card">
                            ...
                            <TableHead className="p-2"><Input placeholder="Country Name" className="h-8 border-border bg-background text-xs" value={colFilters.name} onChange={(e) => setColFilters((f) => ({ ...f, name: e.target.value }))} /></TableHead>
                            <TableHead className="p-2"><Input placeholder="Weight Unit" className="h-8 border-border bg-background text-xs" value={colFilters.weightUnit} onChange={(e) => setColFilters((f) => ({ ...f, weightUnit: e.target.value }))} /></TableHead>
                            <TableHead className="p-2"><Input placeholder="Currency" className="h-8 border-border bg-background text-xs" value={colFilters.currency} onChange={(e) => setColFilters((f) => ({ ...f, currency: e.target.value }))} /></TableHead>
                            <TableHead className="p-2"><Input placeholder="ISD Code" className="h-8 border-border bg-background text-xs" value={colFilters.isdCode} onChange={(e) => setColFilters((f) => ({ ...f, isdCode: e.target.value }))} /></TableHead>
