...
import { ServiceCenterOptionalFloatingAsyncSelect, UserFloatingAsyncSelect } from "@/components/masters/floating-master-async-selects"
...
export function CourierForm({ initialData }: CourierFormProps) {
    ...
    const extraUsers = useMemo(
        () => (initialData?.user ? ([initialData.user] as unknown as UtilityUser[]) : undefined),
        [initialData?.user],
    )

    const extraServiceCenters = useMemo(
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FloatingFormItem required label={<>Linked user</>}>
                                    <FormControl>
                                        <UserFloatingAsyncSelect
                                            triggerRef={field.ref}
                                            disabled={isEdit}
                                            value={field.value > 0 ? field.value : undefined}
                                            onChange={(v) => field.onChange(v ?? 0)}
                                            queryKeyScope={isEdit && initialData ? `courier-${initialData.id}` : 'courier-new'}
